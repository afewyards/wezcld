...
│   └── wezcld            # Wrapper to launch claude with fake $TMUX env
├── lib/
...
Runtime state at `~/.local/state/wezcld/`:
```
...
SHIM_STATE_DIR="$HOME/.local/state/wezcld"
...
### Task 2: Wrapper script (`wezcld`)

**Files:**
- Create: `bin/wezcld`

**Step 1: Write `bin/wezcld`**

```bash
#!/usr/bin/env bash
# Launch Claude Code with WezTerm tmux shim enabled.
# Usage: wezcld [claude args...]
...
if [ "$TERM_PROGRAM" != "WezTerm" ]; then
    echo "wezcld: not running inside WezTerm, launching claude directly" >&2
    exec claude "$@"
fi
...
```
feat: add wezcld launcher wrapper
```
...
handle_display_message() {
    ...
    case "$format" in
        ...
        '#{session_name}:#{window_index}')
            ...
            ;;
        *)
            echo "wezcld: unknown format: $format" >&2
            ;;
    esac
}
...
case "${1:-}" in
    ...
    *)               echo "wezcld: not yet handled: tmux $*" >&2; exit 0 ;;
esac
```
...
# Symlink wezcld
ln -sf "$BIN_DIR/wezcld" "$TARGET/wezcld"
...
echo "Installed:"
echo "  $TARGET/wezcld -> $BIN_DIR/wezcld"
...
echo ""
echo "Usage: wezcld          (instead of 'claude' for agent teams)"
echo ""
...
4. In WezTerm, run `wezcld`
5. Verify `echo $TMUX` → `/fake/wezcld`
...
4. **PATH safety**: Installing a `tmux` shim in `~/.local/bin` affects ALL tmux calls from that shell. The `TERM_PROGRAM` + `WEZTERM_SHIM_LIB` guard should prevent interference, but need to verify no edge cases (e.g., launching real tmux from within a wezcld session).
